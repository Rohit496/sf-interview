...
export default class FlowContainer extends LightningElement {
    ...
    handleFlowStatusChange(event) {
        if (event.detail.status === 'FINISHED') {
            ...
            const resultId = outputVariables.find(
                (v) => v.name === 'outputRecordId'
            );
        }
    }
}
...
public with sharing class FlowAccountActions {
    ...
    public static List<MergeResult> mergeDuplicates(
        List<MergeRequest> requests
    ) {
        ...
        for (MergeRequest req : requests) {
            ...
            try {
                ...
                if (master == null || duplicate == null) {
                    ...
                    Database.MergeResult mergeRes = Database.merge(
                        master,
                        duplicate
                    );
...
public with sharing class FlowExternalCallout {
    ...
    public static List<AddressResult> verifyAddress(
