...
trigger OrderEventTrigger on Order_Event__e(after insert) {
    ...
    EventBus.TriggerContext.currentContext()
        .setResumeCheckpoint(Trigger.new[Trigger.new.size() - 1].ReplayId);
}
```
...
trigger OrderEventTrigger on Order_Event__e(after insert) {
    try {
        ...
        for (Order_Event__e event : Trigger.new) {
            ordersToUpdate.add(
                new Order(Id = event.Order_Id__c, Status = event.Status__c)
            );
        }
...
| Custom (MyObj\_\_c) | `/data/MyObj__ChangeEvent`     |
            ...
            switch on changeType {
                ...
                when 'UPDATE' {
                    ...
                    if (
                        ...
                        changedFields.contains('AnnualRevenue')
                    ) {
                ...
                when 'GAP_CREATE', 'GAP_UPDATE', 'GAP_DELETE', 'GAP_UNDELETE' {
                    ...
                    System.debug(
                        'Gap event — full re-sync needed for: ' + recordIds
                    );
                ...
                when 'GAP_OVERFLOW' {
                    ...
                    System.debug(
                        'GAP_OVERFLOW — re-sync entire object: ' +
                        header.getEntityName()
                    );
