...
public with sharing class ObjectTriggerHandler {
    ...
    @TestVisible
    static Boolean hasRunAfter = false;
...
```apex
trigger ObjectTrigger on SObjectName(
    before insert,
    before update,
    after update
) {
    if (Trigger.isBefore) {
        if (Trigger.isInsert)
            ObjectTriggerHandler.beforeInsert(Trigger.new);
        if (Trigger.isUpdate)
            ObjectTriggerHandler.beforeUpdate(Trigger.new, Trigger.oldMap);
    }
    if (Trigger.isAfter) {
        if (Trigger.isUpdate)
            ObjectTriggerHandler.afterUpdate(Trigger.new, Trigger.oldMap);
    }
}
```
...
### AuraHandledException setMessage() Quirk (CRITICAL)

- **Bug:** `new AuraHandledException('msg')` — calling `getMessage()` on the caught exception returns `"Script-thrown exception"`, NOT the message passed to the constructor.
- **Fix:** Always call `setMessage()` explicitly after construction:
...
- Without `setMessage()`, test assertions on `e.getMessage()` will fail.
