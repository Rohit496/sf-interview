...
## ErrorLogService Pattern (Mar 2026)

- `Database.insert(record, false, AccessLevel.USER_MODE)` on a single record (not a list) is valid -- the `Database.insert` overload accepts SObject or List<SObject>.
- Fire-and-forget logging: do NOT add try/catch to the Apex class itself; `allOrNone=false` is the correct mechanism to silently consume failures. The method intentionally has no try/catch block.
- LWC service modules (no HTML template) must have `<isExposed>false</isExposed>` in their meta.xml -- if this is set to `true`, flag as WARNING (should not appear in App Builder).
- classifyError picklist string values ('Apex', 'Network', 'Validation', 'Unknown') must exactly match the Error_Type\_\_c picklist fullName values -- always cross-check these during review.
- `logErrorToServer` wrapping the Apex call in a top-level try/catch (not just catching the Apex error) is the correct pattern for fire-and-forget client-side logging.
